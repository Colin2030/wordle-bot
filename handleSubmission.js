// handleSubmission.js — rivals + tie-break + robust streaks
// ---------------------------------------------------------
// Notes:
...
// - Reaction now receives `rival` so it can add a cheeky nudge.
//
// Dependencies you already have elsewhere in the project:
//   utils: getAllScores, logScore, getLocalDateString, isMonthlyChampion
//   streakUtils: calculateCurrentAndMaxStreak
//   openaiReaction: generateReaction (newer version supports `rival` param)
//   fallbackreactions: reactionThemes (your non-AI backups)
//   playerProfiles: optional pronouns map
...
function buildTodayLeaderboard(allScores, today, injectPlayer = null, injectScore = 0) {
  ...
  const lastIdx = new Map();     // player -> last seen index (most recent row index for today)
...
module.exports = async function handleSubmission(bot, msg) {
  ...
  if (attempts !== 'X') {
    ...
    const seenYellows = new Set(); // track positions that turned yellow once
    const seenGreens = new Set();  // track positions that finally went green
    ...
    gridLines.forEach((line, i) => {
      ...
      tiles.forEach((tile, idx) => {
        // We only track by index (column); this is coarse but effective
        const key = `${idx}`;
        ...
        if (tile === '🟩') {
          ...
          if (!seenGreens.has(key)) {
            ...
            if (seenYellows.has(key)) {
              finalScore += rule.yellowToGreen; // reward converting a yellow to green later
            }
            seenGreens.add(key);
          }
        } else if (tile === '🟨') {
      ...
      // Row is fully green and not the final row → give a tidy bonus
  ...
  const yestTotals = yestScores.reduce((acc, [_, p, s]) => {
  ...
  const lastWeekTotals = lastWeekScores.reduce((acc, [_, p, s]) => {
    acc[p] = (acc[p] || 0) + parseFloat(s);
    return acc;
  }, {});
